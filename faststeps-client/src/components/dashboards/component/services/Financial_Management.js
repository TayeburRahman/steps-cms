...
import { useGetFinancialContentQuery } from '../../../../features/auth/authApi'
...
function Financial_Management() {
    ...
    const { financial_content } = useSelector((state) => state.auth);

    

    useEffect(() => {
        if (languesBanner === "eng") {
            setBannerData(financial_content?.r1Content?.eng)
        } else {
            setBannerData(financial_content?.r1Content?.arb)
        }
    }, [languesBanner, handelBanner, financial_content])
    ...
    return (
       ...
       <Box sx={{ boxShadow: 1, marginTop: "40px" }} >
           <Container className=''>
               <div className='banner-content '>
                   <p className='pt-2'>Financial.. R-1 settings Text(H2, Normal)</p>
                   ...
                   <div className='pb-3 pt-3'>
                       ...
                       <>
                           {
                               !handelBanner && <TextUpdate langues={languesBanner} content={bannerData} name="financial" row="r1" status={handelBanner} setStatus={setHandelBanner} />
                           }
                       </>
                   </div>
               </div>
       ...
       <Box sx={{ boxShadow: 1, marginTop: "40px" }} >
                <Container className=''>
                    <p className='pt-2'>Financial.. R-2 settings Text(H3, Normal)</p>
                    ...
                    <div className=' text-center mb-2 border-settings'>
                        ...
                        <AddObjects openR2={openR2} setOpenR2={setOpenR2} name="financial" row="r2" />
                    </div>

                    <div className='row'>
                        {
                          financial_content &&  financial_content?.r2Content?.map((data, idx) => (
                                <div key={idx} className='col-sm-12 col-md-6 col-lg-6 pb-1 pt-1'>
                                    ...
                                    <div className='pb-3 pt-3'>
                                        ...
                                        <>
                                            {
                                                r2IdxEdit === idx && <UpdateObject langues={languesR2} objectData={objectData} name="financial" row="r2"  setStatus={setR2IdxEdit} />
                                            }
                                        </>
                                    </div>
                                </div>
