...
import { useGetOfficeContentQuery } from '../../../../features/auth/authApi'
import AddObjects from './AddObjects'
import TextUpdate from './TextUpdate'
import UpdateObject from './UpdateObject'

function Offices_services() {
    ...
    const { office_content } = useSelector((state) => state.auth);

    useEffect(() => {
        if (languesBanner === "eng") {
            setBannerData(office_content?.r1Content?.eng)
        } else {
            setBannerData(office_content?.r1Content?.arb)
        }
    }, [languesBanner, handelBanner, office_content])
    ...
    return (
        <div className="w-100"> 
            ...
            <Box sx={{ boxShadow: 1, marginTop: "40px" }} >
                <Container className=''>
                    <div className='banner-content '>
                        <p className='pt-2'>Office.. R-1 settings Text(H2, Normal)</p>
                        ...
                        <div className='pb-3 pt-3'>
                            ...
                            <>
                                {
                                    !handelBanner && <TextUpdate langues={languesBanner} content={bannerData} name="office" row="r1" status={handelBanner} setStatus={setHandelBanner} />
                                }
                            </>
                        </div>
                    </div>
            ...
            <Box sx={{ boxShadow: 1, marginTop: "40px" }} >
                <Container className=''>
                    <p className='pt-2'>Office.. R-2 settings Text(H3, Normal)</p>
                    ...
                    <div className=' text-center mb-2 border-settings'>
                        ...
                        <AddObjects openR2={openR2} setOpenR2={setOpenR2} name="office" row="r2" />
                    </div>

                    <div className='row'>
                        {
                          office_content &&  office_content?.r2Content?.map((data, idx) => (
                                <div key={idx} className='col-sm-12 col-md-6 col-lg-6 pb-1 pt-1'>
                                    ...
                                    <div className='pb-3 pt-3'>
                                        ...
                                        <>
                                            {
                                                r2IdxEdit === idx && <UpdateObject langues={languesR2} objectData={objectData} name="office" row="r2"  setStatus={setR2IdxEdit} />
                                            }
                                        </>
                                    </div>
                                </div>
...
export default Offices_services
